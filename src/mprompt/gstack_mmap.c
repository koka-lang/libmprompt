...
// forward declaration 
static bool mp_mmap_commit_on_demand(void* addr, mp_gstack_t* g);
...
static bool mp_mmap_commit_on_demand(void* addr, mp_gstack_t* g) {
  ...
  mp_access_t access = mp_gstack_check_access(g, page, &stack_size, &available, NULL);
...
static void mp_sig_handler_commit_on_demand(int signum, siginfo_t* info, void* arg) {
  ...
  if (mp_mmap_commit_on_demand(info->si_addr, mp_gstack_current())) {
    return; // ok!
  }
